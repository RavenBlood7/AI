...
/////////////////////////CONTINUOUS////////////////////////////////////
vector<float> DTreeNode::getPossibleValues(int index)
{
	stringstream tempStream;
	float tempValue;
	DTreeNode * tempSet = this->clone();
	vector<float> possValues;
	for (int i = 0; i < tempSet->getSize(); i++)
	{
		//tempStream.str("");
		tempStream.clear();//clear any bits set
		tempStream.str(std::string());
		tempStream << tempSet->set.front().at(index);
		tempStream >> tempValue;
		if(!(std::find(possValues.begin(), possValues.end(), tempValue) != possValues.end())) 
		{
			possValues.push_back(tempValue);
		} 
		tempSet->popCase();		
	}
	delete tempSet;
...
int DTreeNode::frequencyLessThanEq(int index, float value)
{
	int total = 0;
	stringstream tempStream;
	float tempValue;
...
DTreeNode* DTreeNode::subsetLessThanEq(int index, float value)
{
	DTreeNode* subset = new DTreeNode();
	vector<string> tempVec;
	int i = 0;
...
DTreeNode* DTreeNode::subsetGreaterThan(int index, float value)
{
	DTreeNode* subset = new DTreeNode();
	vector<string> tempVec;
	int i = 0;
...
void DTreeNode::splitContinuous(string attr, float curValue, int index)
{	
	stringstream tempStream;
	float tempValue;
	tempStream << curValue;
	...
	children->push_back(new DTreeNode());
	children->at(0)->attr = attr;
	children->at(0)->relop = '<';
	children->at(0)->value = tempStream.str();

	children->push_back(new DTreeNode());
	children->at(1)->attr = attr;
	children->at(1)->relop = '>';
	children->at(1)->value = tempStream.str();
	...
	{	
			if (!set.empty())
			{
				tempStream.clear();//clear any bits set
				tempStream.str(std::string());
				tempStream << set.front().at(index);
				tempStream >> tempValue;
			}
			if (!set.empty() && tempValue <= curValue)
			{				
				children->at(0)->addCase(set.front());	
				set.pop_front();
			}
			else
			{
				children->at(1)->addCase(set.front());	
				set.pop_front();				
			}
	}	
}